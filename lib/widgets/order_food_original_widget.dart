...
class OrderFoodOriginalWidget extends StatefulWidget {
  final int index;
  final OrderFood orderFood;

  OrderFoodOriginalWidget({
    required this.index,
    required this.orderFood,
  });

  @override
  _OrderFoodOriginalWidgetState createState() => _OrderFoodOriginalWidgetState();
}

class _OrderFoodOriginalWidgetState extends State<OrderFoodOriginalWidget> {
  ...
  void initState() {
    ...
    super.initState();
    double finalPrice = widget.orderFood.originalPrice;
    widget.orderFood.orderExtraIngredientsList.forEach((element) {
      finalPrice += element.originalPrice;
    });
    widget.orderFood.orderComboOfferIngredientsList.forEach((element) {
      finalPrice += element.originalPrice;
    });
    totalPrice += (finalPrice * widget.orderFood.quantity);
  }
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Row(
                      children: [
                        Text(
                          '${ProjectConstant.currencySymbol}${widget.orderFood.originalPrice.toString()}',
                          style: ProjectConstant.WorkSansFontMediumTextStyle(
                            fontSize: 14,
